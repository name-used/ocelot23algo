...
def train():
    ...
    kernel_filter = gaussian_kernel(size=16, steep=2, device=device)
    ...
    with Timer() as T:
        for i in range(40):
            ...
            for code in trains:
                ...
                divides = []
                gts = []
                while len(images) < train_batch_size:
                    p = int(torch.rand(1))
                    # 30 % 的机率全图随机
                    if p < 0.2:
                        x, y = (torch.rand(2) * 1009).type(torch.int64).clamp(0, 1008)
                    # 30 % 的机率在 label 附近 30 范围随机
                    elif p < 0.3 + 0.3:
                        x0, y0 = label_points[np.random.randint(len(label_points))]
                        x, y = (torch.rand(2) * 61 - 30 + [x0, y0]).type(torch.int64).clamp(0, 1008)
                    # 20 % 的机率在 label 附近 10 范围随机
                    elif p < 0.2 + 0.3 + 0.3:
                        x0, y0 = label_points[np.random.randint(len(label_points))]
                        x, y = (torch.rand(2) * 21 - 10 + [x0, y0]).type(torch.int64).clamp(0, 1008)
                    # 20 % 的机率在 label 附近 3 范围随机
                    else:
                        x0, y0 = label_points[np.random.randint(len(label_points))]
                        x, y = (torch.rand(2) * 7 - 3 + [x0, y0]).type(torch.int64).clamp(0, 1008)
...
class Net(torch.nn.Module):
    ...
    def forward(
            self,
            image: torch.Tensor,  # (b, 3, 16, 16)
            coarse: torch.Tensor,  # (b, 1, 16, 16)
            fine: torch.Tensor,  # (b, 1, 16, 16)
            classify: torch.Tensor,  # (b, 2, 16, 16)
            divide: torch.Tensor,  # (b, 2, 16, 16)
    ):
