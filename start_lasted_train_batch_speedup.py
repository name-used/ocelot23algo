    ...
    with torch.no_grad():
        with Timer() as T:
            ...
            for code in trains:
                ...
                divides = []
                gts = []
                for y in range(0, 1024 - s + 1, s):
                    for x in range(0, 1024 - s + 1, s):
                        ...
                        gts.append(torch.tensor([
                            # # 维度 0 表示点的自信度
                            # label[:, y:y + s, x:x + s].sum().clamp(0, 1),
                            # # 维度 1、2 表示点的类型
                            # label[0, y:y + s, x:x + s].sum().clamp(0, 1),
                            # label[1, y:y + s, x:x + s].sum().clamp(0, 1),

                            # 维度 0 表示点的自信度
                            (label[:, y:y + s, x:x + s] * kernel).sum().clamp(0, 1),
                            # 维度 1、2 表示点的类型
                            (label[0, y:y + s, x:x + s] * kernel).sum().clamp(0, 1),
                            (label[1, y:y + s, x:x + s] * kernel).sum().clamp(0, 1),
                        ], dtype=torch.float32, device=device))
