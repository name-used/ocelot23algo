...
detector = torch.jit.load('./weights/detector-V-Wrong-best-jit.pth')
        ...
        if cache_code and \
            ...
            for y in range(0, height - kernel + 1, step):
                for x in range(0, width - kernel + 1, step):
                    ...
                    wrong_map = divide_heat_map[:, y: y + kernel, x: x + kernel]
                    image = torch.concat([torch.tensor(image, device=device), wrong_map.permute(1, 2, 0)], dim=2)
                    patches.append(image)
                    grids.append((x, y))
            image = torch.stack(patches).type(torch.float32).permute(0, 3, 1, 2)
            # 在这里做升降采样
