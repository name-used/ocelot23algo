    ...
    with torch.no_grad():
        ...
        for y in range(0, height - kernel + 1, step):
            for x in range(0, width - kernel + 1, step):
                ...
                # 预测后处理
                coarse_patch = coarse_patch.softmax(dim=1)[:, 1:, :, :].permute(0, 2, 3, 1)
                fine_patch = fine_patch.clamp(0, 1).permute(0, 2, 3, 1)
                classify_patch = classify_patch.clamp(0, 1).permute(0, 2, 3, 1)
                # 融合
                merger.set(
